...
- `weekly_gross`, `avg_ticket_price`, `seats_in_theatre`, `weeks_running`
...
<p style="font-size:10px"># Main Question: Average Price vs Weekly Gross </p>

```{r avg-price-weekly-gross1, message = FALSE, warning = FALSE, cache = TRUE, echo = FALSE}
grosses %>%
...
# Average Ticket Price vs Weekly Gross
```{r preliminary-graph, message = FALSE, warning = FALSE, echo = FALSE}
grosses %>%
  ggplot(aes(y = weekly_gross,x = avg_ticket_price)) +
  geom_point() +
  geom_smooth(se = FALSE) +
...
# Set recipe and workflow
```{r test-train-fit, message = FALSE, warning = FALSE}
set.seed(11)
...
```{r rsq-rmse,message=FALSE,warning=FALSE,echo=FALSE}
