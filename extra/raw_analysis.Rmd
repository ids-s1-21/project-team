...
improved_model<-
  lm(grosses1$weekly_gross ~ grosses1$avg_ticket_price + 
>>>>>>> 73e168ed9b6635cca5d6ddef3c4ba0e65550f517
       grosses1$avg_ticket_price2 +
...
logxy<-
  lm(gross_log ~ price_log, data=grosses_log)
summary(logxy)
...
glimpse(train_data)
glimpse(test_data)
grosses_fit<-linear_reg()%>%
  set_engine("lm")%>%
  fit(gross_log ~ ., data = train_data)
predict(object = grosses_fit, new_data = test_data)
```
